...
# class BertForSequenceClassificationDebug(BertPreTrainedModel):
...
def get_regression_labels(rel_fed, conversion_table):
    '''
        Gets both the relevance labels and applies the conversion to them, 
        making them applicable for a regression task head.

        Example: conversion_table = [[0, 1, 2, 3, 4],[0.1, 1, 0.9, 0.8, 0.6, 0.5]]
        will let you convert the Corpus relevance labels to a relevance number that be digested by 
        the model, take in mind that the Cranfield relevance level of 1 is the highest, going
        down to 4, and zero is no relevance assigned, which implies irrelevant.
    '''
    ...
    for query_num in range(0, 225):
        ...
        for l in range(5): # a valid conversion table has five pairs
            gold_unsorted_score = [conversion_table[1][l] if x==conversion_table[0][l] else x for x in gold_unsorted_score]
            current_labels[list(gold_unsorted)] = gold_unsorted_score
        current_labels = current_labels[1:]
        labels.append(current_labels)
    return labels
...
def training(model, train_dataloader, device, optimizer, scheduler):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # type(output.loss.item()) is class 'float'
        loss.backward()
